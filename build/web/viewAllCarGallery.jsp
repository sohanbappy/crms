...
<!--Showcase for Car-->
        ...
        <div class="item-carousel" data-toggle="owlcarousel" data-owlcarousel-settings='{"items":4, "pagination":true, "navigation":true, "itemsScaleUp":true}' >
			...
			Connection con = dbConfig.getConnection();
                        ...
                        if(request.getParameter("carKey")!=null){
                            
                            String key=request.getParameter("carKey");
                            
                        rs = st.executeQuery("select * from car_info_tb WHERE status='available' AND car_model LIKE '%"+key+"%'  ");
                        }
                        else if(request.getParameter("type")!=null){
                            
                             if(request.getParameter("type").equals("per_day")){
                            rs = st.executeQuery("select * from car_info_tb WHERE status='available'  AND rent_type='per_day'  ");
                                }
                            else {
                            rs = st.executeQuery("select * from car_info_tb WHERE status='available' AND rent_type='per_hour'  ");
                            }
                       }
                        else{
			rs = st.executeQuery("select * from car_info_tb WHERE status='available' ");
                        }
...
<!--                     <span style="text-align: center;float: left;"><b><a href="viewAllCarGallery.jsp?pageno=<%=k%>" >  <%=k%> </a></b></span>-->
                     &nbsp;&nbsp;&nbsp;&nbsp;
<%
                                        }
                                        st.close();
